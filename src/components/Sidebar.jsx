...
import {
    ...
    MessageCircle, UserPlus, Package, CalendarDays
} from "lucide-react";
...
const navItems = {
    admin: [
        ...
        { to: "/admin/lost-found", label: "Lost & Found", icon: <Package className="w-5 h-5" /> },
        { to: "/admin/facility-management", label: "Facility Mgmt", icon: <CalendarDays className="w-5 h-5" /> },
    ...
    resident: [
        ...
        { to: "/resident/lost-found", label: "Lost & Found", icon: <Package className="w-5 h-5" /> },
        { to: "/resident/facility-booking", label: "Facility Booking", icon: <CalendarDays className="w-5 h-5" /> },
    ...
    security: [
        ...
        { to: "/security/lost-found", label: "Lost & Found", icon: <Package className="w-5 h-5" /> },
...
export default function Sidebar({ open, onClose }) {
    ...
    const roleBadgeColor = {
        admin: "bg-[#E5B94B]/20 text-yellow-500",
    ...
    return (
        <>
            ...
            <aside
                className={`fixed top-0 left-0 z-50 h-full w-[17rem] sidebar sidebar-bg text-white flex flex-col transition-transform duration-300 ease-in-out rounded-tr-2xl rounded-br-2xl
            ...
            >
                ...
                <div className="border-b border-white/5 px-5 py-5">
                    ...
                    <p className="text-[var(--text-caption)] text-gray-400 mt-1">Smart Residential Management</p>
                </div>

                {/* User info */}
                <div className="px-4 py-4 border-b border-white/5">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-[#E5B94B]/20 flex items-center justify-center text-sm font-bold text-yellow-500 flex-shrink-0">
                            {userData?.name ? userData.name.split(" ").map((n) => n[0]).slice(0, 2).join("") : (role ? role[0].toUpperCase() : "U")}
                        </div>
                        <div className="min-w-0 flex-1">
                            <p className="text-[var(--text-small)] font-semibold text-white truncate">
                                {userData?.name || roleLabel[role] || "User"}
                            </p>
                            <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                                <span className={`text-[var(--text-caption)] px-2 py-0.5 rounded-full font-medium ${roleBadgeColor[role] || "bg-white/5 text-gray-400"}`}>
                                    {roleLabel[role] || role}
                                </span>
                                {userData?.apartmentNumber && (
                                    <span className="text-[var(--text-caption)] text-gray-500">• {userData.apartmentNumber}</span>
                                )}
                            </div>
                        </div>
                    </div>
                ...
                <div className="px-4 py-4 border-t border-white/5">
                    <div className="flex items-center gap-2 px-1">
                        <span className="inline-block w-2 h-2 rounded-full bg-emerald-500 flex-shrink-0" aria-hidden />
                        <span className="text-[var(--text-caption)] text-gray-400">All services operational</span>
                    </div>
                </div>
            </aside>
        </>
